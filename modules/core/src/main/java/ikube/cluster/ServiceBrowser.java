...
public final class ServiceBrowser implements Runnable {
	...
	protected InetAddress multicastAddressGroup;
	...
	public void run() {
		while (shouldRun) {
			...
			try {
				...
				logger.info("Recieved packet : " + receivedPacket);
				if (isReplyPacket()) {
					ServiceDescription descriptor;
					// Notes on behavior of descriptors.indexOf(...) ServiceDescriptor objects check for 'equals()' based only on the
					// instanceName field. An update to a descriptor implies we should replace an entry if we already have one. (Instead of
					// bothering with the details to determine new vs. update, just quickly replace any current descriptor.)
