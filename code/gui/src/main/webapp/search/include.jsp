...
<!--
 ...
 -->
	...
	<tr>
		<td>
			<jsp:include page="/search/pagination.jsp" flush="true" /> 
		</td>
	</tr>
	<tr>
		<td>
			<c:forEach var="result" items="${results}">
				...
				Distance: <c:out value="${result['distance']}" /><br>
				Name: <c:out value="${result['name']}" /><br>
				Latitude: <c:out value="${result['latitude']}" /><br>
				Longitude: <c:out value="${result['longitude']}" /><br>
				Index : <c:out value="${indexName}" /><br>
				<%-- <c:if test="${entry.key == 'name' || entry.key == 'path' || entry.key == 'id'}">
					<img 
						alt="Document type" 
						src='<c:url value="/images/icons/todo.gif" />'
						height="15px"
						width="15px"
						align="bottom" />
				</c:if> --%>
