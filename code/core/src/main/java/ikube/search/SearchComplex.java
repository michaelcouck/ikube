...
public class SearchComplex extends Search {
    ...
    public Query getQuery() throws ParseException {
        if (typeFields == null ||
                typeFields.length == 0 ||
                searchStrings.length != typeFields.length ||
                occurrenceFields == null ||
                occurrenceFields.length == 0 ||
                occurrenceFields.length != typeFields.length) {
            if (searchStrings.length != searchFields.length) {
                String message = "Strings and fields must be the same length at least : " +
        ...
        for (int i = 0; i < searchStrings.length; i++) {
            ...
            if (boosts != null && boosts.length > i && boosts[i] > 0) {
                query.setBoost(boosts[i]);
            }
