...
/**
 * Executes a spatial search. One of these classes needs to be instantiated for each search as the member variables
...
public class SearchSpatial extends SearchComplex {
     ...
     * for each search performed, and is certainly not thread safe.
     *
     * @param searcher the searcher, with geolocation data in it, that we will perform
     *                 the distance search on
     */
    ...
    protected TopDocs search(final Query query) throws IOException {
        ...
        try {
            // Strip out the coordinates from the search strings
            // and the field types so the super query will do the
            // complex search and not the multi field search
