...
/**
 * This is the base class for actions. Actions execute logic on index contexts. Actions may include opening the searcher
 * on a new index, indexing or deleting the old indexes. This class is intended to be sub-classed. Common methods in
 * this base class is checking that the index is current, i.e. has not expired and whether the searcher should be
 * re-opened on the new index.
 * 
...
public abstract class Action<E, F> implements IAction<E, F> {
	...
	/**
	 * These are the rules defined for this action. They will be evaluated collectively by the {@link RuleInterceptor}
	 * and the action will be executed depending on the result of the rules.
	 */
	@Transient
	...
	/**
	 * This is the predicate that will be evaluated. The predicate consists of a boolean expression that contains the
	 * individual results of the rules. For example '!IsThisServerWorking && !AnyServersWorking'. The rules' results
	 * will be inserted into the parameter place holders and the expression evaluated by {@link JEP}.
	 */
