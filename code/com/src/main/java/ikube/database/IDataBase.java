...
/**
 * Interface for database objects. Database objects support access to the persistent store. Implementations of this class then act as the generic data access
 * object for all entities, avoiding boiler place specific code for entities which is so popular now and essentially just a pre-determined place to put the
 * database access.
 * 
...
public interface IDataBase {
	...
	/**
	 * Executed a delete statement in the entity manager and returns the number of rows that were affected by the statement, the number of records that were
	 * deleted.
	 * 
	...
	/**
	 * Removes the batch of objects in the parameter list. Depending on the underlying implementation, this method should be executed in a transaction. As such
	 * the commit of the transaction will then flush the removes to the database. So the statements will be batched up and sent in one shot to the database.
	 * 
	...
	/**
	 * Persists a batch of objects, i.e. the list passed in the parameter list. As with the {@link IDataBase#removeBatch(List)} method this will run in a
	 * transaction, and the commit of the transaction will send the statements to the database, depending on the underlying Jpa implementation.
	 * 
	...
	/**
	 * Merges the object parameter with the object from the database. In the case where a primitive field is changed in the object and this change needs to be
	 * persisted the merge will update the database with the new value.
	 * 
	...
	/**
	 * Merges the entities in a batch. Running in a transaction, the underlying implementation will batch up the statements and send them to the database when
	...
	/**
	 * Finds an object by the id field only. This method will be very expensive as each object in the database is iterated through and the id field found by
	 * reflection and compared to the parameter. Also only the first object will be returned. This method assumes that every object in the database has a
	 ...
	 * @param objectId the id of the object
	 * @return the object or null if there is no such object or a runtime exception if there is more than one result, meaning of course that there is a
	 *         constraint violation, like unique indexes etc.
	 */
	...
	/**
	 * This method will find the class type specified in the database, and then sort by multiple fields, in the order that they were specified. the direction of
	 * sort is boolean for ascending and false for descending.
	 * 
	 * @param <T> the type to expect
	 * @param klass the class to select from the database
	 * @param fieldsToSortOn the fields to sort on, in the order of the sort requirements, i.e. first on start time then on end time for example
	 * @param directionOfSort the direction of the sort for each field. For example sort acesnding on the start time and descending on the end time
	...
	/**
	 * Selects a single object based on the sql and the parameters. The combination of parameters should result in a single unique entity being returned from
	 * the database, otherwise a non unique exception should be thrown.
	 * 
	 ...
	 * @return the entity that returned from the query and parameters. This could also result in an entity not found exception and a non unique result exception
	 */
	<T> T find(final Class<T> klass, final String sql, final Map<String, Object> parameters);

	/**
	 * Selects a list of entities from the database based on the sql and the parameters. The results will be limited by the starting position and the maximum
	 * results parameters. This method is therefore suited to paging results. Note that the sql should have an order by clause as the underlying implementation
	...
	/**
	 * Executed the passed sql on the database, returning a single entity. Note that the query should result in exactly one entity otherwise either a no result
	 * exception or a non unique exception will be thrown.
	 * 
	 * @param <T> the type of entity expected
	 * @param klass the class of entity expected
	...
	/**
	 * Executes the query against the database using the sql and parameters dynamically set in the query. As with the above this query should result in a unique
	...
	/**
	 * This method will execute ad-hoc sql, for select, on the database, based on JpaQl like sql. The result from the query depends on the query, for example if
	 * there are multiple sums and averages in the query then it will be an array of numbers, but if the query is something like 'select sum(s.count) from
	 * Search as s where s.indexName = :indexName', then the result will be a single number. There can only be a single result from this query.
	 * 
	...
	/**
	 * This method will execute ad-hoc sql, for update, on the database, based on JpaQl like sql. The result is the result returned from the entity manager. In
	 * the case of a delete the result will be the numbe of records deleted.
	 * 
	 * @param sql the JpaQl to execute on the database
	 * @param names the names of the parameters
	 * @param values the values of the parameters to narrow the results
	...
	/**
	 * This method does a query on the entities using the class type and the parameters are n array format, for the names of the fields to use in the predicate
	 * and the values for those fields in the predicate.
	 * 
	...
	/**
	 * This method will execute a named suery on the database fintered by the parameters which will narrow the results and return a list with of the specified
	 * type, starting at the posisiton specified and with a maximum results of that specified.
	 * 
	 * @param <T> the type to return
	 * @param klass the class of the return type
	 * @param sql the sql to execute on the database, note that this is a named query, i.e. must be defined for the JPA implementation
	 * @param names the names of the fields to use as filter in the query
	 * @param values the values of the fields to use for the filter in the query
	 * @param startPosition the first entity in the result, in the case that this list os sorted by the id for example thsn this will be the next page in the
	 ...
	 * the compiler will not correct and discrepancies in the query prior to execution live.
	 * 
