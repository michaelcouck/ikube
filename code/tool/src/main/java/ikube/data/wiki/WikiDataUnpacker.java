...
public class WikiDataUnpacker {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			} else if (args[0].equals(UNPACK_SINGLES)) {
	...
	protected static void read7ZAndUnpackFiles(final String inputFilePath, final String... outputDisksPaths) throws Exception {
		...
		Future<?> future = ThreadUtilities.submit(null, new WikiDataUnpacker7ZFileWorker(inputFile));
		List<Future<?>> futures = new ArrayList<Future<?>>(Arrays.asList(future));
