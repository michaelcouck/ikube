...
import ikube.web.shared.services.EmployeeServiceAsync;
	...
	public void onModuleLoad() {
		...
		class RetrieveEmployeeHandler implements ClickHandler, KeyUpHandler {
			...
			private void sendEmployeeIdToServer() {
				...
				employeeService.findEmployee(Long.parseLong(textToServer), new AsyncCallback<EmployeeDTO>() {
					public void onFailure(Throwable caught) {
						// Show the RPC error message to the user
						dialogBox.setText("Remote Procedure Call - Failure");
						serverResponseLabel.addStyleName("serverResponseLabelError");
					...
					public void onSuccess(EmployeeDTO employeeDTO) {
						...
						serverResponseLabel.removeStyleName("serverResponseLabelError");
						if (employeeDTO != null)
							serverResponseLabel.setHTML("Employee Information <br>Id : " + employeeDTO.getEmployeeId() + "<br>Name : "
									+ employeeDTO.getEmployeeName() + "<br>Surname : " + employeeDTO.getEmployeeSurname()
									+ "<br>Job Title : " + employeeDTO.getJob());
						else
